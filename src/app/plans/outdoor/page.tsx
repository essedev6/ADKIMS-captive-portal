...
export default function OutdoorPlansPage() {
  ...
  useEffect(() => {
    const fetchPlanPage = async () => {
      try {
        console.log('Fetching outdoor plans...');
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/plan-pages/outdoor`);
  ...
  // Payment handling is now done directly in the button click
            ...
            {pageData.plans.map((plan, index) => (
              ...
              >
                <div className="absolute top-0 right-0 w-20 h-20">
                  <div 
                    className={`absolute transform rotate-45 text-white text-xs py-1 right-[-35px] top-[32px] w-[170px] text-center
                      ${index === 0 ? 'bg-purple-600' : index === 1 ? 'bg-blue-600' : 'bg-indigo-600'}`}
                  >
                ...
                <div className="p-8">
                  <div className="text-center mb-8">
                    ...
                    <div className="flex items-center justify-center mb-4">
                      <span className={`text-4xl font-bold ${index === 0 ? 'text-purple-400' : index === 1 ? 'text-blue-400' : 'text-indigo-400'}`}>
                  ...
                  <div className="space-y-4 mb-8">
                    {plan.bandwidthLimit && (
                      <div className="flex items-center text-gray-300">
                        <svg className={`w-5 h-5 mr-3 ${index === 0 ? 'text-purple-400' : index === 1 ? 'text-blue-400' : 'text-indigo-400'}`} fill="currentColor" viewBox="0 0 20 20">
